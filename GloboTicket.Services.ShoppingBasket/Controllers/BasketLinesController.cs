...
namespace GloboTicket.Services.ShoppingBasket.Controllers
{
    ...
    public class BasketLinesController : ControllerBase
    {
        ...
        public async Task<ActionResult<BasketLine>> Post(Guid basketId, [FromBody] BasketLineForCreation basketLineForCreation)
        {
            ...
            // Populate Event cache for this microservice with the Event being added.
            // Each time a basketLine is added, we check if the event is already in the local database.  
            // If it is not, we retrieve the event from the Event Catalog microservice and add it to the local store.
